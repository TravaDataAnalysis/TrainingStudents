...
class General:
    ...
    def ex2a(self, start_block, end_block):
        events = self.contract.events.Transfer.createFilter(fromBlock=start_block, toBlock=end_block).get_all_entries()
        event_list = []
        for event in events:
            event_list.append(json.loads(self.web3.toJSON(event)))
